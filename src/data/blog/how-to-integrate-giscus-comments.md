...
title: How to integrate Giscus comments into NSC
...
The approach is serverless, as the comments are stored on _GitHub_ and dynamically loaded from there on client side, hence perfect for a static blog, like _NSC_.
...
- specify the _GitHub_ repository you want to connect, typically the repository containing your statically hosted _NSC_ blog on _GitHub Pages_
...
Simply add that to the source code of the site. Most likely, if you're using _NSC_ and want to enable comments on posts, navigate to `PostDetails.astro` and paste it into the desired location where you want the comments to appear, perhaps underneath the `Share this post on:` buttons.
